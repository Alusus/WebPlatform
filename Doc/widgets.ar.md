...
الصنف الأساسي (الجذر) لكل الضبيطات.
...
* `فروع` (`children`): `مـصفوفة[سـندنا[ضـبيطة]]` (`Array[SrdRef[Widget]]`). مصفوفة تحمل الضبيطات التي يحويها هذا الصنف.
...
* `فروع` (`children`) الفروع التي نريد إضافتها و التي هي عبارة عن ضبيطات.
...
* `فرع` (`child`) الضبيطة المراد إضافتها.
...
* `فروع` (`children`): الفروع التي نريد إزالتها و التي هي عبارة عن ضبيطات.
...
* `معطل` (`disabled`): ثـنائي (`bool`). إن كانت قيمتها 1 ستُعطل الضبيطة بحيث لا يستطيع المستخدم تغيير قيمتها.
...
* `فرع` (`child`): `سـندنا[ضـبيطة]` (`SrdRef[Widget]`). العنصر المحتوى ضمن الارتباط التشعبي، مثلاً
