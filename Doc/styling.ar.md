...
الطرز تسمح للمستخدم بالتحكم بمظهر واجهة المستخدم عبر التحكم بالخصائص المظهرية للضبيطات.

### طـراز (Style)

الصنف الرئيسي للتحكم بالخصائص المظهرية لضبيطة معينة في حالة معينة. تتوفر فيه الخصائص التالية:
...
* `طراز_الآوتلاين` (`outlineStyle`) الطرز الخاص بالخط الذي يرسم خارج الإطار وحوله. يستخدم مع ضبيطات
  الإدخال.
...
وهذا يمكن المبرمج من التحكم بشكل الضبيطة المبدئي وشكلها عند تغير حالتها (مثلا عند تمرير الفأرة فوقها)
كما تسمح بالتحكم بشكل فروع تلك الضبيطة عند تغير حالتها. الضبيطات لا تتعامل مباشرة مع الصنف `طـراز`
...
```
// دالة لإنشاء ضبيطة الترويسة
دالة أنشئ_الترويسة():سـندنا[Widget]{
    // هنا نقوم بإنشاء مساحة مستطيلة لوضع مكونات الضبيطة ضمنها
    أرجع صـندوق({}).{
        // الأطرزة المستعملة لهذه الضبيطة
        الطراز.{
        ...
        // مكونات الضبيطة و التي نعتبرها فروع للضبيطة
        أضف_فروع({
...
المؤشر فوق الضبيطة الرئيسية فيُطبق عندها الطراز على الفرع. يتم ذلك بتمرير الحالة إلى `الطراز`
...
صـندوق({}).{
    ...
    // نطبق طراز على ضبيطة فرعية من الفئة أيقونة
    الطراز(">>icon").{
    ...
    // نقوم بتطبيق طراز على ضبيطة فرعية من الفئة أيقونة 
    ...
    أضف_فروع({
        // ضبيطة رابط
        // و الذي يحوي على ضبيطة فرعية هي صورة
